...
def send_notification(sender, instance, **kwargs):
    ...
    if instance.content_type.model == 'blog':
        ...
        verb = '{0} has liked your blog ({1})'.format(instance.user.get_nickname_or_username(), blog.title) 
    elif instance.content_type.model == 'comment':
        comment = instance.content_object
        verb = '{0} has liked your comment "{1}"'.format(instance.user.get_nickname_or_username(),strip_tags(comment.text))
